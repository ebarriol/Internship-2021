...
from tensorflow.keras.layers import Input, UpSampling2D, Dropout, Concatenate
...
def main(args):
    ...
    if mode == 'train':
        ...
        ckpt = ModelCheckpoint(filepath=weights_path, verbose=1, monitor='val"_loss', mode='auto', save_best_only=True)
        model.fit_generator(train_data_gen, train_data_length // batch_size, epochs=nepochs, verbose=1,
                            callbacks=[ckpt], validation_data=val_data_gen, validation_steps=val_steps)
        ...
        for i, img_path in enumerate(img_paths):
            ...
            lt = pd.DataFrame(layers, columns=['Layer Type', 'Layer Name', 'Layer Trainable'])
            display(lt)
        ...
        for idx in tqdm(range(len(dirs))):
            ...
            with TiffFile(fpath) as tif:
                ...
                for tstep in range(len(out_img)):
                    ...
                    curr_out_img = (curr_out_img - np.min(curr_out_img)) / (
                            np.max(curr_out_img) - np.min(curr_out_img)) * 255.0
                ...
                imageio.mimsave(out_dir + '/output_time_series_movie/' + file_prefix + '_movie.gif', movie_list,
                                duration=0.2)
